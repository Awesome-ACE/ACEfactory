...
def prepare_dataloaders(args, tokenizer, logger):
    ...
    # Process datasets
    train_dataset = datasets.load_dataset('csv', data_files={'train': f'{args.dataset_path}/train.csv'})['train']
    train_dataset_token_lengths = [len(item['aa_seq']) for item in train_dataset]
    val_dataset = datasets.load_dataset('csv', data_files={'validation': f'{args.dataset_path}/validation.csv'})['validation']
